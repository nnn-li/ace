...
export function getChars(doc: dcm.Document, pos: { row: number; column: number }): number {
    return getLinesChars(doc.getLines(0, pos.row - 1)) + pos.column;
};

export function getPosition(doc: dcm.Document, chars: number): { row: number; column: number } {
    var i;
    var line: string;
